...
//4. main 함수에서 Cat 객체와 Dog 객체를 생성하고,
// 각 객체의 makeSound() 메소드를 호출하여 결과를 확인하세요.
        Cat cat = new Cat();
        Dog dog = new Dog();
        cat.makdSound(); // 야옹
        dog.makdSound(); // 멍멍
...
//2. Runnable을 구현하여 각각 "사람이 달립니다.", "자동차가 달립니다."를 출력하는
// Person 클래스와 Car 클래스를 만드세요.
//3. main 함수에서 Runnable 타입의 변수 runner를 선언하세요.
//4. runner 변수에 new Person()을 대입하여 run() 메소드를 호출하고,
// 그 다음 new Car()를 대입하여 run() 메소드를 호출하여 결과가 다르게 나오는 것을 확인하세요.
        Runnable runner;
        runner = new Person();
        runner.run(); // 사람이 달립니다
        runner = new Car();
...
//2. DataAccessObject를 구현하여 각각 "Oracle DB에 저장",
// "MySQL DB에 저장"을 출력하는 OracleDao, MySqlDao 클래스를 만드세요.
//3. main 함수에서 DataAccessObject 타입의 변수 dao를 선언하세요.
//4. dao에 new OracleDao()를 대입하여 save()를 호출하고,
// 그 다음 new MySqlDao()를 대입하여 save()를 호출하여 DB가 쉽게 교체되는 것을 확인하세요.
